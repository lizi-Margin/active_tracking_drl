...
try:
    import gym_tvizdoom
    has_gym_tvizdoom = True
except ImportError:
    has_gym_tvizdoom = False

try:
    import gym_tvizdoom2
    has_gym_tvizdoom2 = True
except ImportError:
    has_gym_tvizdoom2 = False

try:
    import gym_unrealcv
    has_gym_unrealcv = True
except ImportError:
    has_gym_unrealcv = False
...
def create_env(env_id, client_id, remotes, **kwargs):
    ...
    # gym_unrealcv env
    if has_gym_unrealcv:
        env_full_name = env_id
        return create_unrealcv_env(env_full_name)

    # gym_tvizdoom2 env?
    if has_gym_tvizdoom2:
        env_full_name = env_id  # treat it as env_id with (possible) args
        if gym_tvizdoom2.is_recognizable_env(env_full_name):
            return create_tvizdoom2_env(env_full_name)

    spec = gym.spec(env_id)

    if has_gym_tvizdoom:
        if spec.tags.get('vizdoom', False):
...
def create_unrealcv_env(env_full_name):
    ...
    env = Vectorize(env)
    env = UnrealCVRescale84x84x3(env)
    env = DiagnosticsInfo(env)
    env = Unvectorize(env)
...
def create_tvizdoom2_env(env_full_name):
    env = gym_tvizdoom2.make_with_args(env_full_name)
    env = Vectorize(env)
    env = DiagnosticsInfo(env)
    env = Unvectorize(env)
    return env
...
def _normalize_value(array, max_value):
    array *= 1.0/max_value
    return array
...
class VizdoomRescale84x84x3(vectorized.ObservationWrapper):
    def __init__(self, env=None):
        ...
        self.is_ob_tuple = isinstance(env.observation_space, spaces.Tuple)
        self.observation_space = Box(0.0, 1.0, [84, 84, 3]) if not self.is_ob_tuple else \
            spaces.Tuple([Box(0.0, 1.0, [84, 84, 3])] + env.observation_space.spaces[1:])

    def _observation(self, obs_n):
        if self.is_ob_tuple:
            return [(_process_frame84x84x3(obs[0]),) + obs[1:] for obs in obs_n]
        else:
            return [_process_frame84x84x3(obs) for obs in obs_n]


class UnrealCVRescale84x84x3(vectorized.ObservationWrapper):
    def __init__(self, env=None):
        super(UnrealCVRescale84x84x3, self).__init__(env)
        self.observation_space = Box(0.0, 1.0, [84, 84, 3])
...
class VizdoomRescale84x84x3Normalize100(vectorized.ObservationWrapper):
    def __init__(self, env=None):
        super(VizdoomRescale84x84x3Normalize100, self).__init__(env)
        assert isinstance(env.observation_space, spaces.Tuple)
        self.observation_space.spaces[0] = Box(0.0, 1.0, [84, 84, 3])
        self.observation_space.spaces[1] = Box(0.0, 2.0, shape=(1,))

    def _observation(self, obs_n):
        return [[_process_frame84x84x3(obs[0]), _normalize_value(obs[1], 100.0)] for obs in obs_n]
